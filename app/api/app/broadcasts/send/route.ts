...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = (await request.json()) as { templateId?: unknown; contactIds?: unknown; name?: unknown };
    const templateId = String(body.templateId ?? "").trim();
    const broadcastName = String(body.name ?? "").trim();
    ...
    const broadcast = await prisma.broadcast.create({
      data: {
        tenantId,
        name: broadcastName || `Broadcast - ${template.name}`,
        status: "SENDING",
